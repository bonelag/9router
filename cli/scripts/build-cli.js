...
function buildCliPackage() {
  ...
  // requiredFiles: when set, treat a traced partial package as incomplete (e.g. sql.js
  // with sql-wasm.js but missing sql-wasm.wasm) and replace from a complete install.
  // Without requiredFiles, missing packages warn and continue (open is best-effort).
  function ensureModuleInBundle(pkg, requiredFiles = []) {
    ...
    // Next.js tracing may create a partial package directory. Replace it with the
    // complete installed package rather than treating existence as proof of readiness.
    if (fs.existsSync(dest)) {
    ...
    if (!isComplete(dest)) {
      ...
      console.warn(`⚠️  Failed to bundle complete ${pkg} package`);
      return;
    }
